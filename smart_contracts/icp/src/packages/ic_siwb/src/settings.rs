...
///
/// The SIWB library needs to be initialized with a `Settings` instance before it can be used. Call the [`crate::init()`] function
/// to initialize the library.
#[derive(Debug, Clone)]
pub struct Settings {
    /// The domain from where the frontend that uses SIWB is served.
    pub domain: String,
    ...
    // Optional runtime features that can be enabled for SIWB.
...
impl SettingsBuilder {
    ...
    /// # Parameters
    ///
    /// * `domain`: The domain from where the frontend that uses SIWB is served.
    /// * `uri`: The full URI, potentially including port number of the frontend that uses SIWB.
    ...
    /// The scheme used to serve the frontend that uses SIWB.
...
fn validate_network(network: Network) -> Result<Network, String> {
    match network {
        Network::Bitcoin => Ok(Network::Bitcoin),
        Network::Testnet => Ok(Network::Testnet),
        Network::Regtest => Ok(Network::Regtest),
        Network::Signet => Ok(Network::Signet),
        _ => {
            return Err(String::from("Unrecognized Network"));
        }
    }
}
...
mod tests {
    ...
    fn test_successful_settings_creation_defaults() {
        ...
        assert_eq!(settings.network, Bitcoin);
    ...
    fn test_valid_uri_formats() {
        let uris = vec!["http://example.com", "https://example.com:4943/path"];
        for uri in uris {
    ...
    fn test_chain_id_zero() {
        let builder =
            SettingsBuilder::new("example.com", "http://example.com", "some_salt").network(Bitcoin);
    ...
    fn test_uri_with_port_numbers() {
        let builder = SettingsBuilder::new("example.com", "http://example.com:4943", "some_salt");
